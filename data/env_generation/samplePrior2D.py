...
def samplePrior(k, imageFileName, xmin=0, xmax=7, normalize = True):
	...
	#scale to range
	if normalize==True:
		s = min_max_scaler.fit_transform(s.reshape(-1,1)).ravel()
	#plot figure
	fig = plt.figure()
...
filenames = ['kernel_8x8_rough.json', 'kernel_8x8_smooth.json']
#number of samples 
samples = 20
...
for n in range(2):
	...
	for sample in range(samples):
		figName = 'images/kernel%i.%i.pdf' % (n, sample)
